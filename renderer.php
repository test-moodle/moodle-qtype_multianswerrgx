...
/**
 * Base class for generating the bits of output common to multianswerrgx
 * (Cloze) questions.
...
class qtype_multianswerrgx_renderer extends qtype_renderer {
    ...
    /**
     * Generate the question's HTML form and controls.
     *
     * @param question_attempt $qa The question attempt instance.
     * @param question_display_options $options Options for displaying the question.
     * @return string HTML for the question's form and controls.
     */
        ...
        foreach ($question->textfragments as $i => $fragment) {
            if ($i > 0) {
                ...
                if (!$questionisvalid) {
                    ...
                    $questionreplacement = qtype_multianswerrgx::deleted_subquestion_replacement();

                    // It is possible that the subquestion index does not exist. When corrupted quizzes (see MDL-54724) are
                    // restored, the sequence column of mdl_quiz_multianswerrgx can be empty, in this case
                    // qtype_multianswerrgx::get_question_options cannot fill in deleted questions, so we need to do it here.
                    $question->subquestions[$index] = $question->subquestions[$index] ?? $questionreplacement;
                }

                $token = 'qtypemultianswerrgx' . $i . 'marker';
    ...
    /**
     * Render the HTML for a subquestion based on its type.
     *
     * Determines the appropriate renderer for the subquestion based on its type and layout.
     *
     * @param question_attempt $qa The question attempt instance.
     * @param question_display_options $options Options for displaying the question.
     * @param int $index The index of the subquestion.
     * @param question_automatically_gradable $subq The subquestion to render.
    ...
    public function subquestion(question_attempt $qa,
        ...
        /** @var qtype_multianswerrgx_subq_renderer_base $renderer */
        $renderer = $this->page->get_renderer('qtype_multianswerrgx', $subrenderer);
        return $renderer->subquestion($qa, $options, $index, $subq);
    }

    /**
     * Get the correct response for a question attempt.
     *
     * @param question_attempt $qa The question attempt instance.
     * @return string An empty string indicating no correct response is defined.
     */
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class qtype_multianswerrgx_subq_renderer_base extends qtype_renderer {
    ...
    /**
     * Render the HTML for a subquestion based on its type and index.
     *
     * @param question_attempt $qa The question attempt instance.
     * @param question_display_options $options Options for displaying the question.
     * @param int $index The index of the subquestion.
     * @param question_graded_automatically $subq The subquestion to render.
     * @return string HTML for the rendered subquestion.
     * @throws coding_exception If an unexpected subquestion type is encountered.
     */
    ...
    protected function feedback_popup(question_graded_automatically $subq,
        ...
        if ($options->rightanswer) {
            if ($subq->qtype->name() == 'regexp' && $fraction > 0 && $fraction < 1) {
                $feedback[] = get_string('bestcorrectansweris', 'qtype_regexp', $rightanswer);
            } else {
    ...
    protected function get_feedback_image(string $icon, string $feedbackcontents): string {
        ...
        $PAGE->requires->js_call_amd('qtype_multianswerrgx/feedback', 'initPopovers');
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_multianswerrgx_textfield_renderer extends qtype_multianswerrgx_subq_renderer_base {

    /**
     * Render HTML for a subquestion in a question attempt.
     *
     * @param question_attempt $qa The question attempt instance.
     * @param question_display_options $options Options for displaying the question.
     * @param int $index The index of the subquestion.
    ...
    public function subquestion(question_attempt $qa, question_display_options $options,
        ...
        foreach ($subq->answers as $ans) {
            ...
            // For a regexp question we base the input field size on the first (correct) answer.
            if ($subq->qtype->name() == 'regexp') {
                break;
            }
        }
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_multianswerrgx_multichoice_inline_renderer
        extends qtype_multianswerrgx_subq_renderer_base {

    /**
     * Render HTML for a specific subquestion in a question attempt.
     *
     * @param question_attempt $qa The question attempt instance.
     * @param question_display_options $options Display options for the question.
     * @param int $index The index of the subquestion.
     * @param question_graded_automatically $subq The subquestion to render.
     * @return string The HTML representation of the subquestion.
     */
    public function subquestion(question_attempt $qa, question_display_options $options,
            $index, question_graded_automatically $subq) {
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_multianswerrgx_multichoice_vertical_renderer extends qtype_multianswerrgx_subq_renderer_base {

    /**
     * Render HTML for a subquestion within a question attempt.
     *
     * @param question_attempt $qa The question attempt instance.
     * @param question_display_options $options Options for how the question should be displayed.
     * @param int $index The index of the subquestion.
     * @param question_graded_automatically $subq The subquestion to render.
     * @return string The HTML content for the subquestion.
     */
    public function subquestion(question_attempt $qa, question_display_options $options,
            $index, question_graded_automatically $subq) {
    ...
    /**
     * Start the HTML wrapper for a choice with a specified class.
     *
     * @param string $class The CSS class to apply to the wrapper.
     * @return string The opening `<div>` tag with the specified class.
     */
    ...
    /**
     * End the HTML wrapper for a choice.
     *
    ...
    /**
     * Start the HTML wrapper for all choices.
     *
     * @return string The opening `<fieldset>` tag with a legend for accessibility.
     */
    protected function all_choices_wrapper_start() {
        $wrapperstart = html_writer::start_tag('fieldset', ['class' => 'answer']);
        $legendtext = $this->get_answer_label('multichoicex', 'qtype_multianswerrgx');
        $wrapperstart .= html_writer::tag('legend', $legendtext, ['class' => 'sr-only']);
        return $wrapperstart;
    }

    /**
     * End the HTML wrapper for all choices.
     *
     * @return string The closing `</fieldset>` tag.
     */
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_multianswerrgx_multichoice_horizontal_renderer
        extends qtype_multianswerrgx_multichoice_vertical_renderer {

    /**
     * Start the HTML wrapper for a choice with a specified class.
     *
     * @param string $class The CSS class to apply to the wrapper.
     * @return string The opening `<div>` tag with the specified class and 'form-check-inline'.
     */
    protected function choice_wrapper_start($class) {
        return html_writer::start_tag('div', ['class' => $class . ' form-check-inline']);
    }
    ...
    /**
     * Start the HTML wrapper for all choices with a fieldset and legend.
     *
     * @return string The opening `<fieldset>` tag with a legend for accessibility.
     */
    protected function all_choices_wrapper_start() {
        $wrapperstart = html_writer::start_tag('fieldset', ['class' => 'answer']);
        $captiontext = $this->get_answer_label('multichoicex', 'qtype_multianswerrgx');
...
/**
 * Class qtype_multianswerrgx_multiresponse_renderer
 *
 * @copyright  2016 Davo Smith, Synergy Learning
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_multianswerrgx_multiresponse_vertical_renderer extends qtype_multianswerrgx_subq_renderer_base {
     ...
     * Start the HTML wrapper for all choices.
     *
     * @return string The opening `<div>` tag with the class 'answer'.
     */
    protected function all_choices_wrapper_start() {
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_multianswerrgx_multiresponse_horizontal_renderer
    extends qtype_multianswerrgx_multiresponse_vertical_renderer {

    /**
     * Start the HTML wrapper for a choice with a specified class.
     *
     * @param string $class The CSS class to apply to the `<td>` tag.
     * @return string The opening `<td>` tag with the specified class.
     */
    protected function choice_wrapper_start($class) {
     ...
     * End the HTML wrapper for a choice.
     *
     * @return string The closing `</td>` tag.
     */
    protected function choice_wrapper_end() {
     ...
     * Start the HTML wrapper for all choices.
     *
     * @return string The opening tags for a table with class 'answer', including `<tbody>` and `<tr>`.
     */
    protected function all_choices_wrapper_start() {
     ...
     * End the HTML wrapper for all choices.
     *
     * @return string The closing tags for the table structure, including `</tr>`, `</tbody>`, and `</table>`.
     */
    protected function all_choices_wrapper_end() {
