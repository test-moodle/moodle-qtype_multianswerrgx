...
use qtype_multianswerrgx;
use qtype_multianswerrgx_edit_form;
...
require_once($CFG->dirroot . '/question/type/multianswerrgx/edit_multianswerrgx_form.php');
...
final class question_type_test extends \advanced_testcase {
    /** @var qtype_multianswerrgx instance of the question type class to test. */
    protected $qtype;
    ...
    /**
     * Retrieves test question data for use in test cases.
     *
     * @return stdClass The test question data.
     */
    protected function get_test_question_data() {
        ...
        $sadata->options->answers[1] = (object) ['answer' => 'Bow-wow', 'fraction' => 0];
        $sadata->options->answers[2] = (object) ['answer' => 'Wiggly worm', 'fraction' => 0];
        $sadata->options->answers[3] = (object) ['answer' => 'Pussy-cat', 'fraction' => 1];
        ...
        $mcdata->options->answers[1] = (object) ['answer' => 'Dog', 'fraction' => 0];
        $mcdata->options->answers[2] = (object) ['answer' => 'Owl', 'fraction' => 1];
        $mcdata->options->answers[3] = (object) ['answer' => '*', 'fraction' => 0];
    ...
    public function test_name(): void {
        $this->assertEquals($this->qtype->name(), 'multianswerrgx');
    }
    ...
    public function test_get_random_guess_score(): void {
        $q = test_question_maker::get_question_data('multianswerrgx', 'twosubq');
    ...
    public function test_get_random_guess_score_with_missing_subquestion(): void {
        ...
        $question = $generator->create_question('multianswerrgx', 'twosubq', ['category' => $category->id]);
        // Add a non-existent subquestion id to the list.
        $sequence = $DB->get_field('question_multianswerrgx', 'sequence', ['question' => $question->id], MUST_EXIST);
        $DB->set_field('question_multianswerrgx', 'sequence', $sequence . ',-1', ['question' => $question->id]);
    ...
    public function test_get_random_guess_score_with_all_missing_subquestions(): void {
        ...
        $questiondata = test_question_maker::get_question_data('multianswerrgx', 'twosubq');
    ...
    public function test_load_question(): void {
        ...
        $fromform = \test_question_maker::get_question_form_data('multianswerrgx');
        ...
        $this->assertEquals('multianswerrgx', $questiondata->qtype);
    ...
    public function test_question_saving_twosubq(): void {
        ...
        $questiondata = \test_question_maker::get_question_data('multianswerrgx');

        $formdata = \test_question_maker::get_question_form_data('multianswerrgx');

        $generator = $this->getDataGenerator()->get_plugin_generator('core_question');
        $cat = $generator->create_question_category([]);

        $formdata->category = "{$cat->id},{$cat->contextid}";
        qtype_multianswerrgx_edit_form::mock_submit((array)$formdata);

        $form = \qtype_multianswerrgx_test_helper::get_question_editing_form($cat, $questiondata);
    ...
    public function test_questiontext_extraction_of_multiplechoice_subquestions_variants(): void {
        ...
        $q = qtype_multianswerrgx_extract_question($questiontext);
     ...
     *
     * @covers \qtype_multianswerrgx::get_question_options
     */
    public function test_get_question_options(): void {
        ...
        $fromform = test_question_maker::get_question_form_data('multianswerrgx', 'twosubq');
        $fromform->category = $category->id . ',' . $syscontext->id;
